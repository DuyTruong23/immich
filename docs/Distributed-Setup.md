...
┌─ LOCAL DEV (máy bạn) ─────────────────────────────────────────────────┐
│                                                                         │
│   Immich Docker :2283          Vite dev :3000                          │
...
|---|---|---|
| poga-v2 | `pnpm dev` → `:3000` | Build static, host CDN |
...
Mở **http://localhost:3000** — Vite proxy `/api` → `IMMICH_SERVER_URL`.
...
| http://localhost:3000 | Trang login / gallery |
